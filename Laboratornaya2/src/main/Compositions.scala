...
// если этот параметр непустой. Возвращает значение "Нет", если этот параметр пуст.
...
object Compositions {
  ...
  def testForComprehension[A, B, C, D](f: A => Option[B])
    ...
    for {
      ...
      b <- f(a)
      c <- g(b)
    } yield h(c) //yield выводит функцию h(c)
  }
}
//Короче говоря, если у нас есть значение типа A, которое может отсутствовать,
// Scala использует экземпляр Option[A] в качестве своего контейнера. Значение
// of Option является либо экземпляром класса caseSome, когда он присутствует,
// либо объектом caseNone, когда его нет.